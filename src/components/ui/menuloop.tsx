            ...
            <ul className={`slide-menu child${level}  ${MENUITEMS.active ? 'double-menu-active' : ''}  `} style={
               ...
               {level <= 1 ? <li className='slide side-menu__label1'>
                ...
                {MENUITEMS.children.map((firstlevel:any)=>
                    <li className={`${firstlevel.menutitle ? 'slide__category' : ''} ${firstlevel?.type == 'empty' ? 'slide' : ''} ${firstlevel?.type == 'link' ? 'slide' : ''} ${firstlevel?.type == 'sub' ? 'slide has-sub' : ''} 
                     ${firstlevel?.active ? 'open' : ''} 
                     ${firstlevel?.selected ? 'active' : ''}`} key={Math.random()}>
